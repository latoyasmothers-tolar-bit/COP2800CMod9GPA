...
public class PenguinAnalyzer {
    ...
    public void readPenguinData(){
        ...
        try(BufferedReader reader = new BufferedReader(new FileReader(FILE_NAME))){
            ...
            while((line = reader.readLine())!=null){
                ...
                if(tokens.lengths>=7){// check for 7 columns
                    try{
    ...
    public void printPenguinData(){
        ...
        for(PenguinData penguion :penguinList){
            System.out.printf("%-5d %-10s %-10.1f %-15.1f %-10.1f %-8s%n",
    ...
    public void writePenguinData(){
        try(PrintWriter writer = new PrintWriter(new FileWriter("PenguinOutput.txt"))){
            writer/printf("%-5s %-10s %-10.1f %-10.1f %-15.1f %-10.1f %-8s%n",
                          penguin.getSampleNumber(), penguin.getSpecies(),
                          penguin.getBillLength(), penguin.getBillDepth(),
                          penguin.getFlipperLength(), penguin.getBodyMass(),
                          penguin.getSex());
